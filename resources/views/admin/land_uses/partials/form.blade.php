<form x-data="{ area: '' }" @area-calculated.window="area = $event.detail; $refs.areaInput.value = area" action="{{ $action }}" method="POST" class="flex h-[calc(100vh-80px)]">
    @csrf
    @if($method !== 'POST')
        @method($method)
    @endif
    ...
    <div class="w-96 min-w-[380px] bg-white border-r border-gray-200 flex flex-col z-10 shadow-lg">
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $landUse->name) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" required placeholder="Contoh: Sawah Pak Budi">
            ...
            <div>
                ...
                <input x-ref="areaInput" type="number" name="area_hectares" value="{{ old('area_hectares', $landUse->area_hectares) }}" step="0.0001" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" readonly>
            ...
            <div>
                ...
                <input type="text" name="owner" value="{{ old('owner', $landUse->owner) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Nama pemilik lahan">
                <x-input-error :messages="$errors->get('owner')" class="mt-2" />
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                <textarea name="description" rows="4" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Deskripsi singkat...">{{ old('description', $landUse->description) }}</textarea>
        ...
        <div class="p-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between gap-3">
            <a href="{{ route('admin.land-uses.index') }}" class="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-white transition text-sm font-medium">
                Batal
            </a>
